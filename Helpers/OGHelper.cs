...
namespace Simplr.OpenGraph.Helpers
{
	public static class OGHelper
	{
		...
		public static string GetMetadata<T>(OGMetadata metadata, IOGType<T> contentMetadata)
		{
			if (metadata == null)
			{
			...
			result += SetMetaProperties(ReplaceValue(metadata, contentMetadata));

			return result;
		}

		private static IList<OGKeyValue> ReplaceValue<T>(OGMetadata metadata, IOGType<T> contentMetadata, string name = "type")
		{
			var result = new List<OGKeyValue>();

			var metadataKeyValue = GetKeyValue(metadata);
			var contentMetadataKeyValue = GetKeyValue(contentMetadata);

			var itemOld = metadataKeyValue.FirstOrDefault(x => x.Name == name);
			var itemNew = contentMetadataKeyValue.FirstOrDefault(x => x.Name == name);
			if (itemOld != null || itemNew != null)
			{
				// replace Content value
				itemOld.Content = itemNew.Content;
				// delete old item
				contentMetadataKeyValue.Remove(itemNew);
			}
			result.AddRange(metadataKeyValue);
			result.AddRange(contentMetadataKeyValue);
